...
export class Slideshow extends Component {
  ...
  async connectedCallback() {
    ...
    // this.current = this.initialSlideIndex || 0;
    
    // const initial = this.initialSlideIndex;
    // this.current = initial;
    // this.select(initial, undefined, { animate: false });
    ...
    let visibleSlidesAmount = this.#updateVisibleSlides();
    
    if (initial >= 0 && this.refs.slides?.[initial]) {
      this.select(initial, undefined, { animate: false });
      visibleSlidesAmount = 1;
    } else if (visibleSlidesAmount === 0 && this.slides?.length) {
      this.select(0, undefined, { animate: false });
      visibleSlidesAmount = 1;
    }
  ...
  async select(input, event, options = {}) {
    ...
    if (requestedIndex === undefined || isNaN(requestedIndex) || requestedIndex === -1) return;
  ...
  set current(value) {
    ...
    if (current && value >= 0) current.textContent = `${value + 1}`;
    ...
    if (previous) previous.disabled = Boolean(!this.infinite && value <= 0);
    if (next) next.disabled = Boolean(
      !this.infinite && slides && value >= slides.length - 1
    );
  }
  ...
  // }
  get initialSlideIndex() {
    if (this.dataset.initialSlide) {
      const index = parseInt(this.dataset.initialSlide, 10);
